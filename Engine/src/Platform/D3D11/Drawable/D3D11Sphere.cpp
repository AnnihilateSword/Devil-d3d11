...
namespace Devil
{
	D3D11Sphere::D3D11Sphere(D3D11Renderer& renderer, DirectX::XMFLOAT3& position, DirectX::XMFLOAT3& rotation, DirectX::XMFLOAT3& scale)
		: m_Position{ position },
		m_Rotation{ rotation },
		m_Scale{ scale }
	{
		...
		{
			const auto model = Geometry::CreateSphere<VertexPosTex, unsigned int>();
			...
			AddStaticBind(std::make_unique<D3D11Texture>(renderer, "./content/textures/brickwall.jpg"));
	...
	void D3D11Sphere::Update(float deltaTime) noexcept
	{
	}

	DirectX::XMMATRIX D3D11Sphere::GetTransformXM() const noexcept
	{
